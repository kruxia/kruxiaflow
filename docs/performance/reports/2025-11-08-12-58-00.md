...
2. **Database**: `streamflow_benchmark` (recommended) or custom via `DATABASE_URL`
...
createdb streamflow_benchmark

# Run database migrations
DATABASE_URL="postgres://streamflow:streamflow_dev@127.0.0.1:5432/streamflow_benchmark" sqlx migrate run

# Seed OAuth client (required for worker authentication)
DATABASE_URL="postgres://streamflow:streamflow_dev@127.0.0.1:5432/streamflow_benchmark" \
...
cargo run --release --bin streamflow serve --port 8080 \
  --database-url postgres://streamflow:streamflow_dev@127.0.0.1:5432/streamflow_benchmark
```
...
```bash
# Run all load tests
cargo test --package streamflow-benchmark --release -- --nocapture

# Run specific scenario
cargo test --package streamflow-benchmark --release test_sequential_workflow_load -- --nocapture
cargo test --package streamflow-benchmark --release test_parallel_workflow_load -- --nocapture
cargo test --package streamflow-benchmark --release test_high_concurrency_load -- --nocapture
cargo test --package streamflow-benchmark --release test_sustained_throughput -- --nocapture
```
