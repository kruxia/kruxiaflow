...
This guide explains how to run performance benchmarks and interpret results for Kruxia Flow.

## Overview

Kruxia Flow includes automated performance testing infrastructure to ensure we meet throughput and latency targets. All performance tests are conducted via the HTTP API to provide apple-to-apples comparison with other workflow platforms like Temporal and Conductor.
...
2. **Database**: `kruxiaflow_profiling` (recommended) or custom via `DATABASE_URL`
3. **OAuth environment variables** set (see below)
4. **Kruxia Flow server** must be running on `localhost:8080`

### Database Setup

```bash
# Create benchmark database
createdb kruxiaflow_profiling

# Run database migrations
DATABASE_URL="postgres://kruxiaflow:kruxiaflow_dev@127.0.0.1:5432/kruxiaflow_profiling" sqlx migrate run

# Seed OAuth client (required for worker authentication)
DATABASE_URL="postgres://kruxiaflow:kruxiaflow_dev@127.0.0.1:5432/kruxiaflow_profiling" \
  cargo run --package kruxiaflow-profiling --bin seed-oauth-client
```

**Note**: The OAuth client credentials must match your environment variables (`KRUXIAFLOW_CLIENT_ID` and `KRUXIAFLOW_CLIENT_SECRET`). The seed script uses these environment variables.

### Starting Kruxia Flow Server

```bash
# Start server with benchmark database
cargo run --release --bin kruxiaflow serve --port 8080 \
  --database-url postgres://kruxiaflow:kruxiaflow_dev@127.0.0.1:5432/kruxiaflow_profiling
```

**Required Environment Variables**:
- `KRUXIAFLOW_CLIENT_ID`: OAuth client ID (e.g., `kruxiaflow-docker-client`)
- `KRUXIAFLOW_CLIENT_SECRET`: OAuth client secret
- `KRUXIAFLOW_OAUTH_RSA_PUBLIC_KEY_PEM`: RSA public key for JWT validation
- `KRUXIAFLOW_OAUTH_RSA_PRIVATE_KEY_PEM`: RSA private key for JWT signing
- `KRUXIAFLOW_OAUTH_TOKEN_ISSUER`: Token issuer URL (e.g., `http://localhost:8080`)
- `KRUXIAFLOW_OAUTH_TOKEN_AUDIENCE`: Token audience (e.g., `kruxiaflow-dev-audience`)
...
```bash
# Run all load tests
cargo test --package kruxiaflow-profiling --release -- --nocapture

# Run specific scenario
cargo test --package kruxiaflow-profiling --release test_sequential_workflow_load -- --nocapture
cargo test --package kruxiaflow-profiling --release test_parallel_workflow_load -- --nocapture
cargo test --package kruxiaflow-profiling --release test_high_concurrency_load -- --nocapture
cargo test --package kruxiaflow-profiling --release test_sustained_throughput -- --nocapture
```
...
# Check server logs
./target/release/kruxiaflow serve --port 8080 --log-level debug
```
