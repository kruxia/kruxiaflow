...
fn save_results(metrics: &PerformanceMetrics, scenario_name: &str) {
    // Check if PROFILING_OUTPUT_DIR environment variable is set
    let output_dir = match env::var("PROFILING_OUTPUT_DIR") {
        Ok(dir) => {
            eprintln!("PROFILING_OUTPUT_DIR found: {}", dir);
            PathBuf::from(dir)
        }
        Err(_) => {
            eprintln!("PROFILING_OUTPUT_DIR not set, skipping results save");
            return; // Silently skip if not set (for normal test runs)
        }
    };
    ...
    if let Err(e) = fs::write(&results_path, json_str) {
        ...
        eprintln!("Successfully wrote results to: {}", results_path.display());
    }
}
...
async fn test_sustained_throughput() {
    ...
    // Run for configurable duration to test sustained performance and detect memory leaks
    // Memory is sampled every 2 seconds by docker-entrypoint-profiling.sh
    // Default: 120 seconds (2 minutes) - can be overridden via SUSTAINED_LOAD_DURATION_SECS
    let duration_secs = env::var("SUSTAINED_LOAD_DURATION_SECS")
        .ok()
    ...
    let scenario_name = format!(
        "Sustained Throughput ({} seconds, 20 concurrent)",
        duration_secs
    );
    metrics.print_report(&scenario_name);
    save_results(&metrics, &scenario_name);
